      ...
      if (chat.userEmail && process.env.SMTP_HOST && process.env.SMTP_USER) {
        try {
          ...
          await transporter.sendMail({
            from: process.env.SMTP_FROM || `"Soporte Stream Store" <${process.env.SMTP_USER}>`,
            to: chat.userEmail,
            ...
            html: `
              <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;">
                <div style="background: #1C5F5C; color: white; padding: 20px; text-align: center;">
                  <h2 style="margin: 0;">Tienes un nuevo mensaje de soporte</h2>
                </div>
                <div style="padding: 30px; background: #ffffff; color: #333333;">
                  ...
                  <div style="text-align: center; margin-top: 35px;">
                    <a href="${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}" style="background: #3ED5CC; color: #1C5F5C; padding: 12px 25px; text-decoration: none; font-weight: bold; border-radius: 8px; font-size: 16px;">Ir a la tienda</a>
                  </div>
                </div>
              </div>
            `
